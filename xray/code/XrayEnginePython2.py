...
class JavaSourceExtractor:
    ...
    def extract_method_code(self, class_name: str, method_name: str) -> str:
        ...
        try:
            ...
            pattern = re.compile(
                rf"(?:(?:public|protected|private|static|final|synchronized|abstract|native)\s+)*",  # Modifiers
                rf"(?:<[\w?,\s<>]+>\s+)?",  # Generic method types <T>
                rf"[\w<>\[\]]+\s+",  # Return type
                rf"{re.escape(method_name)}\s*" , # Method name
                rf"\([^)]*\)\s*",  # Parameters
                rf"(?:throws\s+[\w,\s]+\s*)?",  # Optional throws declaration
                # rf"\\{",                                                                            # Opening brace
                re.MULTILINE
            )
...
class GenericAIClient:
    ...
    def analyze_method(self, class_name: str, method_name: str, source_code: str, parent_context: str,
                       children_signatures: List[str]) -> dict:
    ...
    def build_prompt(self, class_name: str, method_name: str, source_code: str, parent_context: str,
                     children_signatures: List[str]) -> str:
        return f"""
    System: You are an expert Java Architect analyzing code execution flow chains.
    Analyze the following Java method and explain its operation and data transformations.

    Return strictly a valid JSON object matching this schema:
    {{
      "summary": "<Short 1-2 sentence explanation of what this method does>",
      "parameter_data_flow": "<Explain inputs received, transformations applied, and parameters passed to downstream calls>",
      "side_effects": "<Database writes, network calls, state updates, or none>"
    }}
---
CONTEXT:
- Class: {class_name}
- Method: {method_name}
- Caller/Parent Context: {parent_context or "Root Entrypoint"}
- Downstream Calls Triggered inside: {json.dumps(children_signatures)}
...
"""
    ...
    def _call_llm_api(self, prompt: str) -> str:
        """
        === INTEGRATION POINT ===
        Replace this placeholder with your actual AI provider call.
        Example:
            response = openai.ChatCompletion.create(model="gpt-4o", messages=[...])
            return response.choices[0].message.content
        """
        # MOCK RESPONSE for demonstration/testing without API keys:
        return json.dumps({
            "summary": "Processes and validates domain entity, applying core logic before delegating downstream.",
            "parameter_data_flow": "Receives input DTO, maps properties, and passes transformed parameters to next bean.",
            "side_effects": "Executes state transition or repository updates."
        })
